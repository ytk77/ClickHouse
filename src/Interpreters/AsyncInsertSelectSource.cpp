    ...
    LoggerPtr log)
{
    ...
    if (destination)
    {
        ...
        /// Freeze the dependency graph (materialized views) and INSERT authorization before the
        /// SELECT runs, for the synchronous fallback below. Otherwise a concurrent CREATE
        /// MATERIALIZED VIEW ... TO <destination> or REVOKE INSERT could change where the
        /// fallback writes, or whether it is still authorized, after the SELECT already read.
